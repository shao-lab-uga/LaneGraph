...
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(sys.path[0])),versionName,'cnnmodels'))
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(sys.path[0])),versionName,'framework'))
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(sys.path[0])),versionName))
...
class Train(TrainingFramework):
	def __init__(self):
		...
		savePath = os.path.join(os.getcwd(), 'LogTmp', '{}'.format(datetime.now().strftime("%Y_%m_%d_%H_%M_%S")))
	...
	def saveModel(self, step):
		if step % (self.epochsize * 10) == 0:
			self.model.saveModel(os.path.join(self.modelfolder, "model_epoch_%d" % (step // (self.epochsize))))
		return False
	...
	def visualization(self, step, result = None, batch = None):
		...
		if step % SAVE_FIGURE_INTERVAL == 0:
			...
			for i in range(self.batch_size):
				idStr = '_{}_{}_{}'.format(int(step // (self.epochsize)), step, i) # convention: epoch, step, index of figure

				Image.fromarray(((batch[0][i,:,:,:] + 0.5) * 255).astype(np.uint8) ).save(os.path.join(self.validationfolder, "input{}.jpg".format(idStr)))
				Image.fromarray(((batch[1][i,:,:,0]) * 255).astype(np.uint8) ).save(os.path.join(self.validationfolder, "mask{}.jpg".format(idStr)))
				Image.fromarray(((batch[2][i,:,:,0]) * 255).astype(np.uint8) ).save(os.path.join(self.validationfolder, "target{}.jpg".format(idStr)))
				if self.use_sdmap:
					Image.fromarray(((batch[4][i,:,:,0]) * 255).astype(np.uint8) ).save(os.path.join(self.validationfolder, "sdmap{}.jpg".format(idStr)))
				...
				Image.fromarray(direction_img.astype(np.uint8)).save(os.path.join(self.validationfolder, "targe_direction{}.jpg".format(idStr)))
				
				Image.fromarray(((result[1][i,:,:,0]) * 255).astype(np.uint8) ).save(os.path.join(self.validationfolder, "output{}.jpg".format(idStr)))
				
				direction_img[:,:,2] = np.clip(result[1][i,:,:,1],-1,1) * 127 + 127
				direction_img[:,:,1] = np.clip(result[1][i,:,:,2],-1,1) * 127 + 127
				direction_img[:,:,0] = 127
				
				Image.fromarray(direction_img.astype(np.uint8) ).save(os.path.join(self.validationfolder, "output_direction{}.jpg".format(idStr)))
...
if __name__ == "__main__":
	...
	config["logfile"] = os.path.join(trainer.validationfolder,"log_lane_%s.json" % trainer.instance)
