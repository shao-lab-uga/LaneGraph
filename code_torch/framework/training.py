	...
	def run(self, config):
		...
		while True:
			...
			if 'torch' in config["model_name"]:
				input = torch.permute(torch.FloatTensor(batch[0]), (0,3,1,2)).to(model.device)		# need to rearrange order, N,640,640,4->N,4,640,640
				mask = torch.permute(torch.FloatTensor(batch[1]), (0,3,1,2)).to(model.device)  # N,1,640,640
				target = torch.permute(torch.FloatTensor(batch[2]), (0,3,1,2)).to(model.device) # N,1,640,640
				target_normal = torch.permute(torch.FloatTensor(batch[3]), (0,3,1,2)).to(model.device) # N,2,640,640
