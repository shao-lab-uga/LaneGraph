...
def resnet34unet_v3(x, is_training, ch_in = 3, ch_out = 4, ch = 64, feature_out = False, global_feature_func = None, res_n = 34):
	...
	x = aggregate_block(features[2], features[3], enc_dim[2], enc_dim[3], enc_dim[2], "agg1", batchnorm=True) # 2: (None, 40, 40, 256), 3: (None, 20, 20, 512), x: (None, 40, 40, 256)
	#p16 = prediction_conv(x, enc_dim[2], ch_out, "pred16")
	x = aggregate_block(features[1], x, enc_dim[1], enc_dim[2], enc_dim[1], "agg2", batchnorm=True) # 1: (None, 80, 80, 128), x: (None, 40, 40, 256), x: (None, 80, 80, 128)
	#p8 = prediction_conv(x, enc_dim[1], ch_out, "pred8")
	x = aggregate_block(features[0], x, enc_dim[0], enc_dim[1], enc_dim[0], "agg3", batchnorm=True) # 1: (None, 160, 160, 64), x: (None, 80, 80, 128), x: (None, 160, 160, 64)
	#p4 = prediction_conv(x, enc_dim[0], ch_out, "pred4")
	x = aggregate_block(x1, x, ch*2, enc_dim[0], ch, "agg4", batchnorm=True) # x1: (None, 320, 320, 128), x: (None, 160, 160, 64), x: (None, 320, 320, 64)
	#p2 = prediction_conv(x, ch, ch_out, "pred2")
	p1, _, _ = common.create_conv_layer('output', x, ch, ch_out, kx = 7, ky = 7, stride_x = 2, stride_y = 2, is_training = is_training, batchnorm = False, deconv=True, activation = "linear") # x: (None, 320, 320, 64)->p1L: (None, 640, 640, 4)
