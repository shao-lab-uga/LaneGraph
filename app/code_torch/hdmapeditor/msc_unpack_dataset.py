...
def find_centers(node_links, node_coordinates, threshold):
    ...
    def merge_sublists(lists: list[list[int]]):
        merged = []
    ...
    node_links = [[i for i in sublist if i in valid_node_ids] for sublist in node_links]
...
for region in regions:
    ...
    with open(output_folder + f"/link_{counter}.json", "w") as file:
        json.dump([nidmap, localnodes, locallinks, find_centers(nidmap, localnodes)], file, indent=2)  # type: ignore
